...
const App = () => {
  ...
  const handleDelete = (deletedItem) => {
    axios
      .delete("http://localhost:8000/api/products/" + deletedItem.id)
      .then((response) => {
  ...
  const handleLogin = (findUser) => {
    axios
      .put("http://localhost:8000/api/useraccount/login", findUser)
      .then((response) => {
  ...
  const handleNewUser = (addUser) => {
    axios
      .post("http://localhost:8000/api/useraccount", addUser)
      .then((response) => {
  ...
  return (
    <>
      <div className="container">
        ...
        {displayLogin ? null : (
          <button className="button" onClick={showPage}>
            Create Account
          </button>
        )}
        {displayLogin ? null : (
...
>>>>>>> 411fac53c81f381c395931829192a6e96315862b
        ...
        {showProduct ? (
          <div>
            ...
            <New user={user} handleCreate={handleCreate} />
          </div>
        ) : null}
        {products
          .filter((potato) => {
